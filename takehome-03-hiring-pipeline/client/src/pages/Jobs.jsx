...
const fetchJobs = async () => {
  ...
  const handleToggleStatus = async (job) => {
    if (!isRecruiter || !job || processingJobId) return;
    ...
    try {
      const response = await jobsService.updateJob(job.id, {
        title: job.title,
        ...
        currentJobs.map((currentJob) =>
          currentJob.id === job.id
            ? {
              ...currentJob,
              ...(updatedJob || {}),
              status: updatedJob?.status || newStatus,
            }
            : currentJob
        )
      );
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    return String(job.title || '').toLowerCase().includes(term);
  });
              ...
              <tbody className="bg-white divide-y divide-slate-100">
                {filteredJobs.map((job) => (
                  <tr key={job.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {isRecruiter && job.status !== 'archived' ? (
                        <button
                          type="button"
                          onClick={() => handleToggleStatus(job)}
                          disabled={processingJobId === job.id}
                          className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium transition-colors disabled:cursor-wait disabled:opacity-60 ${job.status === 'closed'
                      ...
                      <EmptyState
                        ...
                        message={
                          searchTerm.trim()
                            ...
                            : 'Create your first job opening to start accepting candidates.'
                        }
                      />
                    </td>
                  </tr>
