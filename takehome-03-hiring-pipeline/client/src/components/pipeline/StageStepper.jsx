...
const StageStepper = ({ currentStage, status, rejectedStage }) => {
  ...
  // If rejected, determine where they were rejected.
  // If rejectedStage is unavailable, use currentStage.
  const failIndex = isRejected
    ? getStageIndex(rejectedStage || currentStage)
    : -1;
      ...
      <div className="flex items-center justify-between relative">
        ...
        <div
          ...
          style={{
            width: `${Math.max(
              0,
              ((isRejected ? failIndex : currentIndex) /
                (STAGES.length - 1)) *
              100
            )}%`,
          }}
        ...
        {STAGES.map((stage, index) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`mt-3 text-sm ${textColor} text-center absolute top-10 w-24 -ml-12 left-1/2`}
              >
                {stage}
              </span>
            </div>
