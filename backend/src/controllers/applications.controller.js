...
export const getApplicationSources = async (req, res) => {
  try {
...
export const createApplication = async (req, res) => {
  try {
    ...
    if (!nameRegex.test(candidate_name.trim())) {
      res.status(400).json({ error: 'Candidate name may contain letters, spaces, hyphens, and apostrophes, but not numbers or alphanumeric combinations.' });
      return;
    }
    ...
    const normalizedEmail = candidate_email.trim().toLowerCase();
    ...
    if (existingActive) {
      if (existingActive.stage === 'hired') {
        res.status(409).json({ error: 'This candidate is already hired for this job opening.' });
        return;
      }
      res.status(409).json({ error: 'This candidate already has an active application for this job opening.' });
      return;
    }
...
export const updateApplication = async (req, res) => {
  try {
    ...
    const normalizedEmail = candidate_email.trim().toLowerCase();
    const targetJobId = job_opening_id || existing.job_opening_id.toString();

    if (
      normalizedEmail !== existing.candidate_email.toLowerCase() ||
      targetJobId !== existing.job_opening_id.toString()
    ) {
      const existingActive = await Application.findOne({
        _id: { $ne: id },
    ...
    existing.candidate_email = normalizedEmail;
...
export const unassignInterviewer = async (req, res) => {
  try {
